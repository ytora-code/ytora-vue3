 ...
 * 初始化搜索框的表单model
 */
const createInitialSearchFormModel = (): SysRecycleBinParam => ({})

/**
 * 初始化抽屉的表单model
 */
const createInitialDrawerFormModel = (): SysRecycleBinParam => ({
  deletedBy: '',
 ...
 */
const useCrud = () => {
  const searchFormModel = ref<SysRecycleBinParam>(createInitialSearchFormModel())
  const pageNo = ref<number>(1)
  const pageSize = ref<number>(10)
  const pages = ref<number>()
  const total = ref<number>()
  const records = ref<SysRecycleBinData[]>([])
  ...
  const page = async () => {
    ...
    try {
      const result = await sysRecycleBinApi.page({
  ...
  const drawerFormModel = ref<SysRecycleBinParam>(createInitialDrawerFormModel())
  const drawerTitle = computed(() => (drawerMode.value === 'create' ? '新增回收站' : '编辑回收站'))
  ...
  const openEditDrawer = async (id?: string | number) => {
    ...
    try {
      const result = await sysRecycleBinApi.queryById(id)
      drawerFormModel.value = {
        ...createInitialDrawerFormModel(),
        ...result,
      } as SysRecycleBinParam
    } catch (error) {
      console.error(error)
  ...
  const deleteByIds = async (ids: Array<string | number>, adjustPage = false) => {
    ...
    try {
      await sysRecycleBinApi.deleteByIds(ids)
  ...
  const deleteRow = (row: SysRecycleBinData) => {
    if (row?.id) {
...
export type SysRecycleBinUpsertModel = SysRecycleBinParam
