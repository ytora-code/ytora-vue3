...
const useFileType = () => {
  ...
  const classifyByMime = (mime?: string, filename?: string): FileKind => {
    ...
    // 0. 有些后端字段名叫 ext，但实际传的不是 MIME，而是 "java" / ".java" 这类后缀。
    // 这里先把“明显不是 MIME 的值”按扩展名处理，避免代码文件被错误识别成 unknown。
    if (m && !m.includes('/')) {
      const kind = classifyByExt(m.replace(/^\./, ''))
    ...
    if (m.startsWith('text/')) {
      ...
      if (['html', 'log', 'json', 'code', 'binary'].includes(extKind)) return extKind
      return 'text'
    }
