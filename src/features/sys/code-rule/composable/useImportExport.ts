...
const useImportExport = (options: {
  searchFormModel: Ref<SysCodeRuleParam>
  page: () => Promise<void>
}) => {
  ...
  const downloadTemplate = async () => {
    ...
    try {
      await sysCodeRuleApi.downloadTemplate()
    } catch (error) {
      console.error(error)
    } finally {
  ...
  const submitImport = async (formData: FormData) => {
    ...
    try {
      await sysCodeRuleApi.import(formData)
      closeImportDialog()
      await options.page()
    } catch (error) {
      console.error(error)
  ...
  const exportData = async () => {
    ...
    try {
      await sysCodeRuleApi.export({ ...options.searchFormModel.value })
    } catch (error) {
      console.error(error)
    } finally {
