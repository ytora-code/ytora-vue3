 ...
 * 初始化搜索框的表单model
 */
const createInitialSearchFormModel = (): SysRoleParam => ({})

/**
 * 初始化抽屉的表单model
 */
const createInitialDrawerFormModel = (): SysRoleParam => ({
  roleName: '',
  roleCode: '',
})
 ...
 */
const useCrud = () => {
  const searchFormModel = ref<SysRoleParam>(createInitialSearchFormModel())
  const pageNo = ref<number>(1)
  const pageSize = ref<number>(10)
  const pages = ref<number>()
  const total = ref<number>()
  const records = ref<SysRoleData[]>([])
  ...
  const page = async () => {
    ...
    try {
      const result = await sysRoleApi.page({
  ...
  const drawerFormModel = ref<SysRoleParam>(createInitialDrawerFormModel())
  ...
  const openEditDrawer = async (id?: string | number) => {
    ...
    try {
      const result = await sysRoleApi.queryById(id)
  ...
  const deleteByIds = async (ids: Array<string | number>, adjustPage = false) => {
    ...
    try {
      await sysRoleApi.deleteByIds(ids)
  ...
  const deleteRow = (row: SysRoleData) => {
    if (row?.id) {
...
export type SysRoleUpsertModel = SysRoleParam
