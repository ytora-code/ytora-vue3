 ...
 * 初始化搜索框的表单model
 */
const createInitialSearchFormModel = (): SysRoleDataScopeParam => ({})

/**
 * 初始化抽屉的表单model
 */
const createInitialDrawerFormModel = (): SysRoleDataScopeParam => ({
  roleId: '',
  scopeId: '',
})
...
const useCrud = () => {
  ...
  const searchFormModel = ref<SysRoleDataScopeParam>(createInitialSearchFormModel())
  const pageNo = ref<number>(1)
  const pageSize = ref<number>(10)
  const pages = ref<number>()
  const total = ref<number>()
  const records = ref<SysRoleDataScopeData[]>([])
  ...
  const page = async () => {
    ...
    try {
      const result = await sysRoleDataScopeApi.page({
  ...
  const drawerFormModel = ref<SysRoleDataScopeParam>(createInitialDrawerFormModel())
  const drawerTitle = computed(() =>
    drawerMode.value === 'create' ? '新增角色-数据范围关系' : '编辑角色-数据范围关系',
  )
  ...
  const openEditDrawer = async (id?: string | number) => {
    ...
    try {
      const result = await sysRoleDataScopeApi.queryById(id)
      drawerFormModel.value = {
        ...createInitialDrawerFormModel(),
        ...result,
      } as SysRoleDataScopeParam
    } catch (error) {
      console.error(error)
  ...
  const submitDrawer = async () => {
    ...
    try {
      await sysRoleDataScopeApi.upsert({
        ...drawerFormModel.value,
      } as SysRoleDataScopeParam)
      drawerVisible.value = false
      await page()
  ...
  const deleteByIds = async (ids: Array<string | number>, adjustPage = false) => {
    ...
    try {
      await sysRoleDataScopeApi.deleteByIds(ids)
  ...
  const deleteRow = (row: SysRoleDataScopeData) => {
    if (row?.id) {
...
export type SysRoleDataScopeUpsertModel = SysRoleDataScopeParam
