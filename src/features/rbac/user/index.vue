...
import UserRoleDrawer from '@/features/rbac/user/UserRoleDrawer.vue'
import type UserRoleDrawerExpose from '@/features/rbac/user/type/UserRoleDrawerExpose'
import useCrud from '@/features/rbac/user/composable/useCrud'
import useImportExport from '@/features/rbac/user/composable/useImportExport'
 ...
 * 表单和表格结构元数据
 */
const { searchFormSchemas, drawerFormSchemas, loadDrawerFormSchemas, drawerRules, tableSchemas } =
  useSchema()
...
const userRoleDrawerRef = ref<UserRoleDrawerExpose | null>(null)

const openUserRoleDrawer = async (row: SysUserData) => {
  await userRoleDrawerRef.value?.open(row)
}

const handleOpenCreateDrawer = async () => {
  openCreateDrawer()
  await loadDrawerFormSchemas(drawerFormModel.value)
}

const handleOpenEditDrawer = async (id?: string) => {
  await openEditDrawer(id)

  if (!drawerVisible.value || !drawerFormModel.value.id) {
    return
  }
...
<template>
  <div>
    <PageLayout>
      ...
      <template #toolbar>
        <div class="toolbar-actions">
          <n-button type="success" size="small" ghost @click="handleOpenCreateDrawer">
            新 增
          </n-button>
          ...
          <n-button type="error" size="small" ghost>回收站</n-button>
        </div>
      </template>
        ...
        >
          <template #action="{ row }">
            <div flex justify-center gap-2>
              <n-button type="primary" ghost size="small" @click="handleOpenEditDrawer(row.id)">
                编辑
              </n-button>
              <n-button type="success" ghost size="small" @click="openUserRoleDrawer(row)">
                角色
              </n-button>
    ...
    <UserRoleDrawer ref="userRoleDrawerRef" />
  </div>
</template>
