...
const useImportExport = (options: {
  searchFormModel: Ref<SysDynamicApiParam>
  ...
  const downloadTemplate = async () => {
    ...
    try {
      await sysDynamicApiApi.downloadTemplate()
    } catch (error) {
      console.error(error)
    } finally {
  ...
  const submitImport = async (formData: FormData) => {
    ...
    try {
      await sysDynamicApiApi.import(formData)
      closeImportDialog()
      await options.page()
    } catch (error) {
      console.error(error)
  ...
  const exportData = async () => {
    ...
    try {
      await sysDynamicApiApi.export({
        ...options.searchFormModel.value,
        groupId: options.currentGroupId?.value?.trim() || undefined,
      })
    } catch (error) {
      console.error(error)
    } finally {
